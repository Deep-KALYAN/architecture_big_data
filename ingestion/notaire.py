...
TMP_PDFS    = Path("data/bronze/notaire")
...
#                 return session
#         except Exception:
#             log.warning("Cache cookies corrompu ou illisible, nettoyage...")
...
#     while True:
#         # Implémentation d'une boucle de retry robuste face aux blocages et expirations
#         for attempt in range(3):
#             try:
#                 r = session.get(url, params={"deedDate": "", "offset": offset, "limit": PAGE_SIZE}, timeout=15)
                
#                 # Gestion explicite du Rate Limiting (429)
#                 if r.status_code == 429:
#                     sleep_time = (5 * (attempt + 1)) + random.uniform(2, 5)
#                     log.warning(f"Rate limited (429) sur {enterprise_number}. Attente de {sleep_time:.1f}s (Tentative {attempt+1}/3)...")
#                     time.sleep(sleep_time)
#                     continue
                    
#                 r.raise_for_status()

#                 # Gestion d'expiration de session (Réponse HTML au lieu de JSON)
#                 if "application/json" not in r.headers.get("content-type", ""):
#                     log.warning(f"[{enterprise_number}] Réponse non-JSON — Session expirée mid-run. Renouvellement de la session...")
#                     # Force la régénération de nouveaux tokens F5 en supprimant le cache périmé
#                     COOKIE_FILE.unlink(missing_ok=True)
#                     session = get_session()
#                     session.headers["Referer"] = f"{BASE}/enterprise/{enterprise_number}/statutes"
#                     continue # Retente l'appel avec la session fraîchement renouvelée
                
#                 # Succès ! Extraction des données JSON
#                 data = r.json()
#                 break
                
#             except requests.exceptions.RequestException as e:
#                 if attempt == 2:
#                     log.error(f"Abandon après 3 tentatives sur {enterprise_number}. Erreur: {e}")
#                     return []
...
#         offset += PAGE_SIZE
#         # Délai de courtoisie entre les pages d'une même entreprise
...
#             )
#             if r.status_code == 429:
#                 sleep_time = 5 + random.uniform(2, 4)
#                 time.sleep(sleep_time)
#                 continue
#             if r.status_code == 404:
#                 return None
...
#             log.warning(f"Erreur lors du téléchargement du PDF (Tentative {attempt+1}/3): {e}")
#             time.sleep(2)
...
#         pdf_path = download_statute_pdf(session, enterprise_number, statute, dest_dir)
#         results.append({**statute, "local_pdf": str(pdf_path) if pdf_path else None})
#         # Délai entre deux téléchargements de fichiers
#         time.sleep(random.uniform(0.4, 1.0))
        
#     # Un délai de politesse global est appliqué à la fin de l'extraction de l'entreprise
